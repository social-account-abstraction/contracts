...
export class Create2Factory {
  ...
  async deployFactory (signer?: Signer): Promise<void> {
    console.log('await this._isFactoryDeployed()', await this._isFactoryDeployed())
    if (await this._isFactoryDeployed()) {
      return
    }
    console.log('signer ?? this.signer', await (signer ?? this.signer).getAddress())
    ...
    const res = await this.provider.sendTransaction(Create2Factory.factoryTx)
    console.log('res', res)
    if (!await this._isFactoryDeployed()) {
  ...
  async _isFactoryDeployed (): Promise<boolean> {
    console.log('this.factoryDeployed', this.factoryDeployed)
    console.log('Create2Factory.contractAddress', Create2Factory.contractAddress)
